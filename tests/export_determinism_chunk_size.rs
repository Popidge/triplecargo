...
fn determinism_across_chunk_sizes_and_line_count() {
    ...
    let base_args = [
        ...
        "winloss",
        "--threads",
        "3", // ensure multi-worker
        "--max-depth",
        "2", // keep test fast; ignored in trajectory but harmless
    ];
    ...
    assert_eq!(
        f_def, f_c1,
        "default chunk-size output must equal chunk-size=1 output"
    );
    assert_eq!(
        f_def, f_c64,
        "default chunk-size output must equal chunk-size=64 output"
    );
    ...
    assert_eq!(
        count_lines(&p_def),
        expected_lines,
        "default chunk-size line count mismatch"
    );
    assert_eq!(
        count_lines(&p_c1),
        expected_lines,
        "chunk-size=1 line count mismatch"
    );
    assert_eq!(
        count_lines(&p_c64),
        expected_lines,
        "chunk-size=64 line count mismatch"
    );
    ...
    assert!(
        s_def_out.contains("[export] chunk_size="),
        "expected chunk_size log for default run"
    );
    assert!(
        s_c1_out.contains("[export] chunk_size="),
        "expected chunk_size log for chunk-size=1 run"
    );
    assert!(
        s_c64_out.contains("[export] chunk_size="),
        "expected chunk_size log for chunk-size=64 run"
    );
    ...
    assert_eq!(
        f_c64, f_c64_b,
        "repeat runs must be byte-identical with same seed and flags"
    );
}

#[test]
fn determinism_across_chunk_sizes_soft_exact() {
    let tmp = tempdir().expect("tmpdir");
    let p_def = tmp.path().join("out_default_soft_exact.jsonl");
    let p_c1 = tmp.path().join("out_chunk1_soft_exact.jsonl");
    let p_c64 = tmp.path().join("out_chunk64_soft_exact.jsonl");
    ...
    cmd_def.arg("--export").arg(&p_def);
    cmd_def.args(&base_args);
    cmd_def.assert().success();

    let mut cmd_c1 = assert_cmd::Command::cargo_bin("precompute").expect("bin");
    cmd_c1.arg("--export").arg(&p_c1);
    cmd_c1.args(&base_args);
    cmd_c1.arg("--chunk-size").arg("1");
    cmd_c1.assert().success();

    let mut cmd_c64 = assert_cmd::Command::cargo_bin("precompute").expect("bin");
    cmd_c64.arg("--export").arg(&p_c64);
    cmd_c64.args(&base_args);
    cmd_c64.arg("--chunk-size").arg("64");
    cmd_c64.assert().success();

    let f_def = fs::read(&p_def).expect("read default soft_exact");
    let f_c1 = fs::read(&p_c1).expect("read chunk1 soft_exact");
    let f_c64 = fs::read(&p_c64).expect("read chunk64 soft_exact");
    assert_eq!(
        f_def, f_c1,
        "soft_exact output must be identical for chunk-size default vs 1"
    );
    assert_eq!(
        f_def, f_c64,
        "soft_exact output must be identical for chunk-size default vs 64"
    );
}
